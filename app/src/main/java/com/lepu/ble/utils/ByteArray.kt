 ...
 * 转n个字节byte数组（大端模式）
 */
fun longToByteArrayBig(value: Long, len: Int): ByteArray {
    val data = ByteArray(len)
    for (i in 0 until len) {
        data[i] = (value shr ((len-1-i)*8) and 0xff).toByte()
    }
    return data
}
/**
 * 转n个字节byte数组（小端模式）
 */
fun longToByteArray(value: Long, len: Int): ByteArray {
    val data = ByteArray(len)
    for (i in 0 until len) {
        data[i] = (value shr (i*8) and 0xff).toByte()
    }
    return data
}
/**
...
fun int3ByteArray(value: Int): ByteArray {
    val b1 = (value and 0xff).toByte()
    val b2 = (value shr 8 and 0xff).toByte()
    val b3 = (value shr 16 and 0xff).toByte()
    return byteArrayOf(b1).plus(b2).plus(b3)
}
/**
 * 转四个字节byte数组（小端模式）
 */
